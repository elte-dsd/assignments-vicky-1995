...
def Ensemble():
  for i in range(500):
    ...
    B +=i
      ...
      B +=i
      static_classifier = base_classifier(e, buffer_class0,  buffer_class1, I, L, D, wd, ws)
      K = 1
      dc = static_classifier
      dynamic_classifier.append(dc)
    else:
      i = (p-1) % I
      DCIR0 = dcir(0)
      DCIR1 = dcir(1)
      train_on_instance(B, i, L, D, wd, ws, DCIR0, DCIR1)
      i = (i+1) % I
      if i == 0:
        k = k + 1
        dc = base_classifier(e, buffer_class0,  buffer_class1, I, L, D, wd, ws)
        if k > D:
          del dynamic_classifier[0]
          dynamic_classifier.append(dc)
          weights(D)
          DCIR0 = dcir(0)
          DCIR1 = dcir(1)
